 ...
 * @param {...Any} [passback=undefined] - The arguments to pass back to the build function.
 */
 ...
 * @param {boolean} [force=false] - Whether files should be copied unconditionally.
 */
exports.copyDirectory = async (source, target, deep = true, force = false) => {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        if (!files[i].endsWith(".old")) {
            ...
            copyTasks.push(namespace.copyFile(source + "/" + files[i], target + "/" + files[i]));
        }
    }
