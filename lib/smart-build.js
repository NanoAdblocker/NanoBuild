 ...
 */
exports.smartBuildFile = async (dependencies, output, build, ...passback) => {
    let stats = [];
 ...
 */
exports.smartCopyFile = async (source, target) => {
 ...
 */
exports.smartCopyDirectory = async (source, target, deep = true) => {
    ...
    for (let i = 0; i < tasks.length; i++) {
        ...
        if (tasks[i].isDirectory()) {
            if (deep) {
                ...
                await exports.smartCopyDirectory(source + "/" + files[i], target + "/" + files[i]);
            }
            continue;
        }

        assert(tasks[i].isFile());
        copyTasks.push(exports.smartCopyFile(source + "/" + files[i], target + "/" + files[i]));
    }
